...
RUN apk update && apk add neovim neovim-doc nodejs npm yarn ripgrep git gcc libstdc++-dev g++ fd rustup curl py3-pip
RUN apk add --update --no-cache --repository http://dl-cdn.alpinelinux.org/alpine/edge/testing/ lua-language-server
RUN yarn global add typescript typescript-language-server vscode-langservers-extracted yaml-language-server @prisma/language-server dockerfile-language-server-nodejs bash-language-server prettier pyright
